...
def convert_to_onehot(x):
    """Convert a tensor to one-hot encoding with 5 classes."""
    return AsDiscrete(to_onehot=5)(x)
...
def main():
    ...
    training_data_filename =  os.path.join(os.getcwd(), "BBBC039/metadata/training.txt")
    image_data_dir = os.path.join(os.getcwd(), "BBBC039 /images/")
    label_data_dir = os.path.join(os.getcwd(), "BBBC039 masks/")
    print(f"training_data_filename: {training_data_filename}")
    ...
    with open(training_data_filename, 'r') as file:
        ...
        for line in file:
            ...
            training_data_dicts.append({
                "image": image_data_dir + image_file_name,
                "label": label_data_dir + label_file_name,
            })

    validation_data_filename =  os.path.join(os.getcwd(), "BBBC039/metadata/validation.txt")
    print(f"validation_data_filename: {validation_data_filename}")
    ...
    train_transforms = Compose(
        [
            ...
            *augmentations,  # Add selected augmentations
            ...
            # Make sure labels have only one channel - use Lambdad to select first channel
            Lambdad(keys=["label"], func=select_first_channel),
            # Convert to one-hot - use named function to avoid pickling issues
            Lambdad(keys=["label"], func=convert_to_onehot),
        ]
    )
    ...
    # kavya: changed the spatial dims to 2
    # kavya: changed the out_channels to 4
    # Create model
    model = UNet(
        spatial_dims=2,
        in_channels=1, 
    ...
    # kavya: changed to_onehot_y to False
    # kavya: changed to sigmoid=True
    if args.loss == "Dice":
        ...
        class_weights = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device)  # Equal weights to start
        ...
        loss_function = FocalLoss(to_onehot_y=True, weight=class_weights, gamma=2.0, softmax=True)  # Default gamma
    ...
    loss_function = DiceCELoss(to_onehot_y=False, softmax=True, lambda_ce=0.5, lambda_dice=0.5)

    # Define metrics
    # For predictions: apply softmax and then get the argmax to get class indices, then convert to one-hot
    post_pred = Compose([
        Activations(softmax=True),
        AsDiscrete(argmax=True, to_onehot=5)
    ])
    
    # For labels: they're already in one-hot format, so no need to convert
    post_label = Identity()
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                val_outputs = sliding_window_inference(
                    val_inputs, spatial_size, 4, model
                )
                ...
                val_pred = post_pred(val_outputs)
                val_label = post_label(val_labels)
    ...
    with torch.no_grad():
        ...
        classes = ['Background', 'Structure 1', 'Structure 2', 'Structure 3', 'Structure 4']
