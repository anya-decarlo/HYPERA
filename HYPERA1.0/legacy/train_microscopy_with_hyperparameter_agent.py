...
from preprocess_microscopy_data import PreprocessMicroscopyData
...
def main():
    ...
    # training_data_filename =  os.path.join(os.getcwd(), "HYPERA/BBBC039/metadata/training.txt")
    # image_data_dir = os.path.join(os.getcwd(), "HYPERA/BBBC039 /images/")
    # label_data_dir = os.path.join(os.getcwd(), "HYPERA/BBBC039 masks/")
    ...
    # validation_data_filename =  os.path.join(os.getcwd(), "HYPERA/BBBC039/metadata/validation.txt")
    ...
    preprocessor = PreprocessMicroscopyData()

    train_files = preprocessor.getTrainingFiles()
    val_files = preprocessor.getValidationFiles()
    ...
    train_transforms = Compose(
        [
            ...
            Spacingd(keys=["image", "label"], pixdim=(1.0, 1.0), mode=("bilinear", "nearest")), # kavya:delete or change it to 1x1
    ...
    # kavya: changed the spatial dims to 2, coz they are 2d images
    ...
    if args.loss == "Dice":
        ...
        loss_function = FocalLoss(to_onehot_y=False, weight=class_weights, gamma=2.0, 
                                  sigmoid=True
                                  )  # Default gamma
    ...
    for epoch in range(args.epochs):
        ...
        for batch_data in train_loader:
            ...
            optimizer.zero_grad()
            print("inputs:", inputs.size()) # torch.Size([4, 1, 64, 64, 64]) --> [4,64,64,64] --> [256,1,64,64]
            # inputs1= torch.reshape(inputs, (256,1,64,64))
            # [batch_size, num_frames, channels, height, width] to [batch_size * num_frames, channels, height, width]
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                print("b4 val_outputs: ", len(val_outputs))
                val_outputs = [post_pred(i) for i in decollate_batch(val_outputs)]
                print("af val_outputs: ", len(val_outputs))
                print("b4 val_labels: ", len(val_labels))
                val_labels = [post_label(i) for i in decollate_batch(val_labels)] 
                # throwing an error here, AssertionError: labels should have a channel with length equal to one.
                # print("af val_labels: ", val_labels.length())
